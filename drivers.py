...
def commandListener(commandQueueHandle : queue, coordinates : list):
	...
	while commandListener_running:
		...
		for xyz in coordinates:
			print(xyz)
			cmd = f"set_pos:{xyz[0]}:{xyz[1]}"
			print("CL>> {}".format(cmd))
			commandQueueHandle.put(cmd)
			

			while initial_X != xyz[0] and initial_Y != xyz[1]:
				print(initial_X)
				print(initial_Y)
				break
				
			
			if xyz == coordinates[len(coordinates)-1]:
				print("hereee")
				cmd = "q"
				commandQueueHandle.put(cmd)
				break
...
def update_stepper(step_pin:int, dt:float):
	...
	print("Moter updated high ", dt)
	sleep(dt/2)
	print("Motor Updated low", dt)
...
def main(coordinates):
	...
	# queues
	commandQueueHandle = queue.Queue(maxsize = len(coordinates))
	...
	# create the threads
	commandListenerThread = threading.Thread(target=commandListener, args=(commandQueueHandle,coordinates))
